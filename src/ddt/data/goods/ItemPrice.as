package ddt.data.goods
{
   ...
   public class ItemPrice
   {
      ...
      public function addItemPrice(param1:ItemPrice, param2:Boolean = false, param3:int = 0) : void
      {
         ...
         for each(_loc4_ in param1.pricesArr)
         {
            this.addPrice(_loc4_,param2,param3);
         }
      }
      ...
      public function toString(param1:Boolean = false) : String
      {
         ...
         if(this.moneyValue > 0)
         {
            if(param1)
            {
               _loc2_ += this.moneyValue.toString() + Price.BANDMONEYTOSTRING;
            }
            else
            {
         ...
         if(this.bandDdtMoneyValue > 0)
         {
            _loc2_ += this.bandDdtMoneyValue.toString() + Price.NEWDDTMONEYTOSTRING;
         }
         if(this.ddtPetScoreValue > 0)
         {
            _loc2_ += this.ddtPetScoreValue.toString() + Price.PETSCORETOSTRING;
         }
         for(_loc3_ in this._prices)
         {
            if(_loc3_ != Price.MONEYTOSTRING && _loc3_ != Price.GOLDTOSTRING && _loc3_ != Price.GESTETOSTRING && _loc3_ != Price.DDTMONEYTOSTRING && _loc3_ != Price.PETSCORETOSTRING)
            {
               _loc2_ += this._prices[_loc3_].toString() + _loc3_;
            }
         }
         return _loc2_;
      }
      ...
      public function toStringI() : String
      {
         ...
         if(this.bandDdtMoneyValue > 0)
         {
            _loc1_ += this.bandDdtMoneyValue.toString() + " " + Price.BANDMONEYTOSTRING;
         }
         for(_loc2_ in this._prices)
         {
            if(_loc2_ != Price.MONEYTOSTRING && _loc2_ != Price.GOLDTOSTRING && _loc2_ != Price.GESTETOSTRING && _loc2_ != Price.BANDMONEYTOSTRING)
            {
               _loc1_ += _loc2_;
            }
         }
