   ...
   public class BallInfo
   {
      ...
      public function loadCraterBitmap() : void
      {
         ...
         if(!BallManager.hasBombAsset(this.craterID))
         {
            if(this.craterID != 0)
            {
               _loc1_ = LoadResourceManager.Instance.createLoader(PathManager.solveCrater(this.craterID),BaseLoader.BITMAP_LOADER);
               _loc1_.addEventListener(LoaderEvent.COMPLETE,this.__craterComplete);
               LoadResourceManager.Instance.startLoad(_loc1_);
               _loc2_ = LoadResourceManager.Instance.createLoader(PathManager.solveCraterBrink(this.craterID),BaseLoader.BITMAP_LOADER);
