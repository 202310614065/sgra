package ddt.command
{
   ...
   public class QuickBuyFrame extends Frame
   {
      ...
      {
         var _loc1_:ShopItemInfo = ShopManager.Instance.getMoneyShopItemByTemplateID(this._view.ItemID);
         this._unitPrice = _loc1_.getItemPrice(1).moneyValue;
      }
      ...
      private function doPay(param1:Event) : void
      {
         ...
         else
         {
            ...
            _loc9_ = [];
            _loc10_ = 0;
            while(_loc10_ < this._view.stoneNumber)
            {
               _loc3_.push(this._shopItemInfo.GoodsID);
               _loc4_.push(this._view.time);
               _loc5_.push("");
               _loc6_.push(false);
               _loc7_.push("");
               _loc8_.push(-1);
               _loc9_.push(this._view.isBand);
               _loc10_++;
            }
            SocketManager.Instance.out.sendBuyGoods(_loc3_,_loc4_,_loc5_,_loc8_,_loc6_,_loc7_,this._buyFrom,null,_loc9_);
         }
         dispatchEvent(new ShortcutBuyEvent(this._view._itemID,this._view.stoneNumber));
         var _loc2_:ItemTemplateInfo = ItemManager.Instance.getTemplateById(this._view._itemID);
         if(_loc2_.CategoryID == 11)
         {
            if(_loc2_.TemplateID == 100100)
            {
               PlayerManager.Instance.dispatchEvent(new BagEvent(BagEvent.GEMSTONE_BUG_COUNT,null));
            }
            else if(_loc2_.TemplateID != 112150)
            {
