package ddt.manager
{
   ...
   public class EffortManager extends EventDispatcher
   {
      ...
      private var _honorNumber:int = 0;
      ...
      private function __initializeAchievementData(param1:CrazyTankSocketEvent) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            ...
            if(Boolean(this.allEfforts[_loc4_.ID]))
            {
      ...
      private function __userRank(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(PathManager.facebookEnable && ExternalInterface.available)
         {
            if(this._honorNumber != 0)
            {
               OverSeasCommunController.instance().facebookSendToFeed(LanguageMgr.GetTranslation("community.facebook.feed.getTitle",this.honorArray[_loc2_ - 1]));
            }
            this._honorNumber = this.honorArray.length;
         }
      }
      ...
      public function getTopEffort(param1:EffortInfo) : int
      {
         ...
         var _loc2_:Array = [];
         var _loc3_:int = this.getLastID(param1);
         var _loc4_:int = -1;
         var _loc5_:EffortInfo = this.getEffortByID(_loc3_);
         while(!this.isTopEffort(_loc5_))
         {
            _loc3_ = this.getLastID(_loc5_);
            _loc5_ = this.getEffortByID(_loc3_);
         }
         return _loc3_;
      }
      ...
      private function getLastID(param1:EffortInfo) : int
      {
         ...
         while(_loc4_ < _loc2_.length)
         {
            if(_loc2_[_loc4_].toString().length > 0)
            {
      ...
      public function getCompleteNextEffort1(param1:int) : Array
      {
         ...
         while(true)
         {
            if(!Boolean(this.completeEfforts[param1 + _loc4_]))
            {
               return _loc2_;
            }
      ...
      public function getCompleteNextEffort(param1:int) : Array
      {
         ...
         for each(_loc5_ in this.completeEfforts.list)
         {
            if(Boolean(_loc5_) && _loc5_.ID != param1)
            {
               _loc3_ = _loc5_.PreAchievementID.split(",");
               _loc6_ = 0;
               while(_loc6_ < _loc3_.length)
               {
                  if(!this.isTopEffort(_loc5_))
                  {
                     if(_loc4_ == int(_loc3_[_loc6_]))
                     {
                        _loc2_.push(_loc5_);
                        _loc4_ = _loc5_.ID;
                     }
      ...
      public function getFellNextEffort(param1:int) : Array
      {
         ...
         while(true)
         {
            if(!Boolean(this.allEfforts[param1 + _loc4_]))
            {
               return _loc2_;
            }
      ...
      private function updateProgress(param1:EffortProgressInfo) : void
      {
         ...
         if(Boolean(_loc2_) && Boolean(_loc2_[0]))
         {
            _loc2_.sortOn("ID");
         }
         var _loc6_:int = 0;
      ...
      private function __lookUpEffort(param1:CrazyTankSocketEvent) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            ...
            if(Boolean(this.allEfforts[this.tempCompleteID[_loc3_]]) && this.isTopEffort(this.allEfforts[this.tempCompleteID[_loc3_]]))
            {
               this.tempInCompleteTopEfforts.add(this.tempCompleteID[_loc3_],this.allEfforts[this.tempCompleteID[_loc3_]]);
            }
            if(Boolean(this.allEfforts[this.tempCompleteID[_loc3_]]))
            {
      ...
      public function getTempCompleteNextEffort(param1:int) : Array
      {
         ...
         while(true)
         {
            if(!Boolean(this.tempCompleteEfforts[param1 + _loc4_]))
            {
               return _loc2_;
            }
      ...
      {
         if(param1.module == UIModuleTypes.DDT_EFFORT)
         {
            UIModuleSmallLoading.Instance.hide();
            UIModuleSmallLoading.Instance.removeEventListener(Event.CLOSE,this.__onClose);
            UIModuleLoader.Instance.removeEventListener(UIModuleEvent.UI_MODULE_COMPLETE,this.__onUIModuleComplete);
