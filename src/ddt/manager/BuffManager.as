         ...
         if(!_effectLoaded)
         {
            if(_templateInfoLoaded)
            {
               for each(_loc1_ in _buffTemplateInfo)
               {
                  ...
                  {
                     LoadResourceManager.Instance.creatAndStartLoad(PathManager.solvePetSkillEffect(_loc1_.EffectPic),BaseLoader.MODULE_LOADER);
                  }
               }
               _effectLoaded = true;
            }
            else
            {
               ...
               _loc3_ = LoadResourceManager.Instance.createLoader(PathManager.solveRequestPath("PetSkillElementInfo.xml"),BaseLoader.TEXT_LOADER,_loc2_);
