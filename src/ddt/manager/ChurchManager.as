package ddt.manager
{
   ...
   public class ChurchManager extends EventDispatcher
   {
      ...
      private function reqeustPayHander(param1:CrazyTankSocketEvent) : void
      {
         var _loc2_:String = null;
         var _loc3_:int = param1.pkg.readByte();
         if(_loc3_ == ChargePackageType.Request_For_Divorce)
         {
            this.isUnwedding = true;
            this._userID = param1.pkg.readInt();
            _loc2_ = param1.pkg.readUTF();
            this.money = param1.pkg.readInt();
            this.unwedingmsg = LanguageMgr.GetTranslation("ddt.friendPay.action",_loc2_,this.money);
      ...
      private function reponseHander(param1:FrameEvent) : void
      {
         ...
         else if(param1.responseCode == FrameEvent.ESC_CLICK || param1.responseCode == FrameEvent.CANCEL_CLICK || param1.responseCode == FrameEvent.CLOSE_CLICK)
         {
         ...
         _loc2_.removeEventListener(FrameEvent.RESPONSE,this.reponseHander);
         _loc2_.dispose();
         _loc2_ = null;
      }
      ...
      private function __marryPropGet(param1:CrazyTankSocketEvent) : void
      {
         ...
         PlayerManager.Instance.Self.IsGotRing = _loc5_;
         if(_loc3_)
         {
      ...
      private function __roomLogin(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(!_loc3_)
         {
            ...
            if(MailManager.Instance.linkChurchRoomId != -1 && (_loc7_ == 5 || _loc7_ == 6))
            {
               StateManager.setState(StateType.DDTCHURCH_ROOM_LIST);
            ...
            else if(_loc7_ == 7)
            {
               ...
               this._linkServerInfo = ServerManager.Instance.getServerInfoByID(_loc8_);
               if(Boolean(this._linkServerInfo))
               {
                  _loc9_ = LanguageMgr.GetTranslation("ddt.church.serverInFail",this._linkServerInfo.Name,MailManager.Instance.linkChurchRoomId);
                  _loc10_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("tips"),_loc9_,"",LanguageMgr.GetTranslation("cancel"),true,true,false,2);
                  _loc10_.addEventListener(FrameEvent.RESPONSE,this.__tipsMarryRoomframeResponse);
               }
               else
               {
         ...
         ChurchManager.instance.currentScene = _loc6_ == 1 ? false : true;
      ...
      private function __tipsMarryRoomframeResponse(param1:FrameEvent) : void
      {
         ...
         _loc2_.removeEventListener(FrameEvent.RESPONSE,this.__tipsMarryRoomframeResponse);
