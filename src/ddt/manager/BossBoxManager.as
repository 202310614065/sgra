package ddt.manager
{
   ...
   public class BossBoxManager extends EventDispatcher
   {
      ...
      public function setupBoxInfo(param1:UserBoxInfoAnalyzer) : void
      {
         ...
         this.timeBoxListHigh = param1.timeBoxListHigh;
      ...
      private function _getTimeBox(param1:CrazyTankSocketEvent) : void
      {
         ...
         if(_loc3_)
         {
            ...
            {
               this._findBoxIdByTime_III(_loc4_);
            }
            else
            {
      ...
      private function _findBoxIdByTime_III(param1:int) : void
      {
         ...
         for each(_loc5_ in this.timeBoxListHigh)
         {
            if(_loc5_.Condition > param1 && _loc5_.Level >= _loc4_)
            {
               if(_loc3_ == null)
               {
                  _loc3_ = _loc5_;
               }
               else if(_loc3_.Condition + _loc3_.Level > _loc5_.Condition + _loc5_.Level)
               {
                  _loc3_ = _loc5_;
               }
            }
         ...
         if(Boolean(_loc3_))
         {
            this._delayBox = _loc3_.ID;
            this._delayBoxHigh = _loc3_.ID;
      ...
      private function _findBoxIdByTime_II(param1:int) : void
      {
         ...
         if(Boolean(_loc2_))
         {
            ...
            this._delayBoxHigh = this._findByTime(_loc2_.Condition);
            this.startDelayTimeB = true;
         }
         else
         {
      ...
      private function _findByTime(param1:int) : int
      {
         var _loc2_:TimeBoxInfo = null;
         for each(_loc2_ in this.timeBoxListHigh)
         {
            if(param1 == _loc2_.Condition && _loc2_.Level > PlayerManager.Instance.Self.Grade)
            {
               return _loc2_.ID;
            }
         }
      ...
      private function _findGetedBoxGrade(param1:int, param2:int) : Boolean
      {
         ...
         for each(_loc4_ in this.gradeBoxList)
         {
            ...
            {
               if(_loc4_.Level > param1 && _loc4_.Level <= param2 && Boolean(_loc4_.Condition))
               {
      ...
      private function _findGetedBoxByTimeI(param1:int) : void
      {
         var _loc2_:TimeBoxInfo = null;
         for each(_loc2_ in this.timeBoxListHigh)
         {
      ...
      private function _getShowBoxID(param1:String) : Array
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc2_:int = 0;
         while(_loc2_ < this._boxShowArray.length)
         {
            ...
            {
               _loc3_ = int(String(this._boxShowArray[_loc2_]).split(",")[0]);
               _loc4_ = int(String(this._boxShowArray[_loc2_]).split(",")[2]);
      ...
      private function _getShowBoxIDI(param1:String) : int
      {
         var _loc3_:int = 0;
         var _loc2_:int = 0;
         while(_loc2_ < this._boxShowArray.length)
         {
            if(String(this._boxShowArray[_loc2_]).indexOf(param1) > 0)
            {
               return int(String(this._boxShowArray[_loc2_]).split(",")[2]);
            }
            _loc2_++;
         }
         return 0;
      ...
      public function showTimeBox() : void
      {
         ...
         if(!this._isBoxShowedNow)
         {
            ...
            if(_loc1_[0] != 0)
            {
               if(PlayerManager.Instance.Self.Grade > this.timeBoxList[1].Level)
               {
                  if(PlayerManager.Instance.Self.Grade > this.timeBoxListHigh[_loc1_[1]].Level)
                  {
                     this._delayBoxHigh = this._findByTime(this.timeBoxListHigh[_loc1_[1]].Condition);
                  }
                  this._showBox(0,this._delayBoxHigh,this.inventoryItemList[this.timeBoxListHigh[this._delayBoxHigh].TemplateID]);
               }
               else
               {
                  this._showBox(0,_loc1_[0],this.inventoryItemList[this.timeBoxList[_loc1_[0]].TemplateID]);
               }
            }
            else
            {
               ...
               if(PlayerManager.Instance.Self.Grade > this.timeBoxList[1].Level)
               {
                  ...
                  _loc2_.goodsList = this.inventoryItemList[this.timeBoxListHigh[this._delayBoxHigh].TemplateID];
               }
               else
               {
      ...
      public function set receiebox(param1:int) : void
      {
         ...
         {
            this._findBoxIdByTime_III(param1);
         }
         else
         {
      ...
      public function set currentGrade(param1:int) : void
      {
         ...
         else
         {
            for each(_loc3_ in this.timeBoxList)
            {
               if(this._currentGrade > _loc3_.Level)
               {
                  this.startDelayTimeB = false;
                  this._isTimeBoxOver = true;
                  this.boxButtonShowType = SmallBoxButton.showTypeHide;
