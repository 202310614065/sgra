package ddt.manager
{
   ...
   public class AcademyManager extends EventDispatcher
   {
      ...
      private function __apprenticeSystemAnswer(param1:CrazyTankSocketEvent) : void
      {
         ...
         switch(_loc2_)
         {
            case AcademyPackageType.ACADEMY_FOR_APPRENTICE:
               ...
               if(SharedManager.Instance.unAcceptAnswer.indexOf(_loc3_) < 0)
               {
            ...
            case AcademyPackageType.ACADEMY_FOR_MASTER:
               ...
               if(SharedManager.Instance.unAcceptAnswer.indexOf(_loc6_) < 0)
               {
            ...
            case AcademyPackageType.UPDATE_APP_STATES:
               ...
               {
                  if(_loc9_ == -1 && PlayerManager.Instance.Self.apprenticeshipState == AcademyManager.APPRENTICE_STATE)
                  {
                     OverSeasCommunController.instance().facebookSendToFeed(LanguageMgr.GetTranslation("community.facebook.feed.firstApprentice"));
                  }
               }
               if(Boolean(this._myAcademyPlayers) && _loc9_ != -1)
               {
      ...
      public function loadAcademyMemberList(param1:Function, param2:Boolean = true, param3:Boolean = false, param4:int = 1, param5:String = "", param6:int = 0, param7:Boolean = true, param8:Boolean = false) : void
      {
         ...
         var _loc10_:BaseLoader = LoadResourceManager.Instance.createLoader(PathManager.solveRequestPath("ApprenticeshipClubList.ashx"),BaseLoader.REQUEST_LOADER,_loc9_);
