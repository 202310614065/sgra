   ...
   public class BuffControl extends Sprite implements Disposeable
   {
      ...
      private function initBuffButtons() : void
      {
         ...
         while(_loc1_ <= 5)
         {
            if(_loc1_ == 4)
            {
               _loc3_ = BuffButton.createBuffButton(_loc1_,this._str);
               _loc3_.CanClick = false;
               this._buffList.addChild(_loc3_);
            }
            else
            {
               _loc3_ = BuffButton.createBuffButton(_loc1_);
               if(this._str == "")
               {
               }
            }
            this._buffBtnArr.push(_loc3_);
            _loc1_++;
         }
         this.setInfo(this._buffData);
         this._growHelpBuff.buffArray = this._buffBtnArr;
         for(_loc2_ in this._buffData)
         {
            if(this._buffData[_loc2_] != null)
            {
               if(this._buffData[_loc2_].Type >= 74 && this._buffData[_loc2_].Type <= 80)
               {
         ...
         for(_loc2_ in param1)
         {
            ...
            switch(param1[_loc2_].Type)
            {
               ...
               case BuffInfo.DOUBLE_PRESTIGE:
                  this._buffBtnArr[4].info = param1[_loc2_];
                  break;
               case BuffInfo.Caddy_Good:
               case BuffInfo.Save_Life:
               case BuffInfo.Agility:
      ...
      private function __addBuff(param1:DictionaryEvent) : void
      {
         ...
         switch(_loc2_.Type)
         {
            ...
            case BuffInfo.PREVENT_KICK:
               ...
               break;
            case BuffInfo.DOUBLE_PRESTIGE:
               this.setBuffButtonInfo(4,_loc2_);
               break;
            case BuffInfo.Caddy_Good:
            case BuffInfo.Save_Life:
            case BuffInfo.Agility:
      ...
      private function __removeBuff(param1:DictionaryEvent) : void
      {
         switch((param1.data as BuffInfo).Type)
         {
            ...
            case BuffInfo.PREVENT_KICK:
               ...
               break;
            case BuffInfo.DOUBLE_PRESTIGE:
               this._buffBtnArr[4].info = new BuffInfo(BuffInfo.DOUBLE_PRESTIGE);
         }
      }
