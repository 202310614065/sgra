      ...
      public function setInfo(param1:BasePlayer, param2:Boolean = true, param3:Boolean = false) : void
      {
         ...
         if(param1.ID == PlayerManager.Instance.Self.ID)
         {
            if(param2)
            {
               ...
               if(this._isVip)
               {
                  if(param1.VIPLevel < 12)
                  {
                     _loc4_ = ServerConfigManager.instance.VIPExpNeededForEachLv[this._level] - param1.VIPExp;
                     this._tipData = LanguageMgr.GetTranslation("ddt.vip.vipIcon.upGradDays",_loc4_,this._level + 1);
                  }
                  else
                  {
