package ddt.view.chat
{
   ...
   public class ChatInputView extends Sprite
   {
      ...
      public function getCurrentInputChannel() : int
      {
         ...
         switch(ChatManager.Instance.state)
         {
            ...
            case ChatManager.SUPER_WINNER_ROOM:
               _loc1_ = int(SUPERWINNER_CHAT);
               break;
            case ChatManager.CHAT_CHRISTMAS_ROOM:
               _loc1_ = int(CHRISTMAS_CHAT);
         }
         return _loc1_;
      }
      ...
      private function __onInputTextChanged(param1:ChatEvent) : void
      {
         ...
         _loc2_.zoneID = PlayerManager.Instance.Self.ZoneID;
         if(_loc2_.channel == ChatInputView.CURRENT)
         {
            if(Boolean(RoomManager.Instance.current))
            {
               if(RoomManager.Instance.isTransnationalFight() && ChatManager.Instance.output.isInGame())
               {
         ...
         {
            _loc2_.zoneID = this._inputField.privateChatInfo.areaID;
            _loc2_.zoneName = this._inputField.privateChatInfo.areaName;
         }
      ...
      {
         if(param1.channel == ChatInputView.PRIVATE && param1.receiver == PlayerManager.Instance.Self.NickName && param1.zoneID == PlayerManager.Instance.Self.ZoneID)
         {
