package com.wirelust.as3zlib
{
   ...
   public class Deflate
   {
      ...
      {
         this.put_byte_withInt(param1 >> 8);
         this.put_byte_withInt(param1 & 0xFF);
      }
      
      final public function send_code(param1:int, param2:Array) : void
      {
         var _loc3_:int = param1 * 2;
         this.send_bits(param2[_loc3_] & 0xFFFF,param2[_loc3_ + 1] & 0xFFFF);
      }
      ...
      public function send_bits(param1:int, param2:int) : void
      {
         ...
         {
            _loc4_ = param1;
            this.bi_buf |= _loc4_ << this.bi_valid & 0xFFFF;
         ...
         {
            this.bi_buf |= param1 << this.bi_valid & 0xFFFF;
      ...
      public function compress_block(param1:Array, param2:Array) : void
      {
         ...
         if(this.last_lit != 0)
         {
            do
            {
               _loc3_ = this.pending_buf[this.d_buf + _loc5_ * 2] << 8 & 0xFF00 | this.pending_buf[this.d_buf + _loc5_ * 2 + 1] & 0xFF;
               _loc4_ = this.pending_buf[this.l_buf + _loc5_] & 0xFF;
               _loc5_++;
               if(_loc3_ == 0)
               {
               ...
               {
                  _loc6_ = int(Tree._length_code[_loc4_]);
                  this.send_code(_loc6_ + LITERALS + 1,param1);
                  _loc7_ = int(Tree.extra_lbits[_loc6_]);
                  if(_loc7_ != 0)
                  {
                  ...
                  _loc7_ = int(Tree.extra_dbits[_loc6_]);
                  if(_loc7_ != 0)
                  {
      ...
      {
         this._tr_flush_block(this.block_start >= 0 ? this.block_start : -1,this.strstart - this.block_start,param1);
      ...
      public function deflate_stored(param1:int) : int
      {
         ...
         this.flush_block_only(param1 == Z_FINISH);
         if(this.strm.avail_out == 0)
         {
            return param1 == Z_FINISH ? FinishStarted : NeedMore;
         }
         return param1 == Z_FINISH ? FinishDone : BlockDone;
      }
      ...
      internal function fill_window() : void
      {
         ...
         while(true)
         {
            ...
            else if(this.strstart >= this.w_size + this.w_size - MIN_LOOKAHEAD)
            {
               ...
               do
               {
                  _loc2_ = this.head[--_loc3_] & 0xFFFF;
               ...
               do
               {
                  _loc2_ = this.prev[--_loc3_] & 0xFFFF;
      ...
      public function deflate_fast(param1:int) : int
      {
         ...
         while(true)
         {
            ...
            if(this.lookahead >= MIN_MATCH)
            {
               ...
               _loc2_ = int(this.head[this.ins_h]);
            ...
            if(this.match_length >= MIN_MATCH)
            {
               ...
               if(this.match_length <= this.max_lazy_match && this.lookahead >= MIN_MATCH)
               {
                  ...
                  do
                  {
                     ++this.strstart;
                     this.ins_h = (this.ins_h << this.hash_shift ^ this.window[this.strstart + (MIN_MATCH - 1)] & 0xFF) & this.hash_mask;
                     _loc2_ = this.head[this.ins_h] & 0xFFFF;
                     this.prev[this.strstart & this.w_mask] = this.head[this.ins_h];
                     this.head[this.ins_h] = Cast.toShort(this.strstart);
                  }
            ...
            {
               _loc3_ = this._tr_tally(0,this.window[this.strstart] & 0xFF);
               --this.lookahead;
               ++this.strstart;
            }
            if(_loc3_)
      ...
      public function deflate_slow(param1:int) : int
      {
         ...
         while(true)
         {
            ...
            if(_loc2_ != 0 && this.prev_length < this.max_lazy_match && (this.strstart - _loc2_ & 0xFFFF) <= this.w_size - MIN_LOOKAHEAD)
            {
               if(this.strategy != Z_HUFFMAN_ONLY)
               {
                  this.match_length = this.longest_match(_loc2_);
            ...
            {
               _loc3_ = this._tr_tally(0,this.window[this.strstart - 1] & 0xFF);
               if(_loc3_)
               {
                  this.flush_block_only(false);
               }
      ...
      internal function longest_match(param1:int) : int
      {
         ...
         var _loc7_:int = this.strstart > this.w_size - MIN_LOOKAHEAD ? this.strstart - (this.w_size - MIN_LOOKAHEAD) : 0;
         var _loc8_:int = this.nice_match;
         var _loc9_:int = this.w_mask;
         var _loc10_:int = this.strstart + MAX_MATCH;
         var _loc11_:uint = uint(this.window[_loc3_ + _loc6_ - 1]);
         var _loc12_:uint = uint(this.window[_loc3_ + _loc6_]);
         ...
         do
         {
            ...
            if(!(this.window[_loc4_ + _loc6_] != _loc12_ || this.window[_loc4_ + _loc6_ - 1] != _loc11_ || this.window[_loc4_] != this.window[_loc3_] || this.window[++_loc4_] != this.window[_loc3_ + 1]))
            {
               ...
               if(_loc5_ > _loc6_)
               {
                  ...
                  _loc11_ = uint(this.window[_loc3_ + _loc6_ - 1]);
                  _loc12_ = uint(this.window[_loc3_ + _loc6_]);
               }
            }
         }
         while(param1 = this.prev[param1 & _loc9_] & 0xFFFF, param1 > _loc7_ && --_loc2_ != 0);
      ...
      internal function deflateSetDictionary(param1:ZStream, param2:ByteArray, param3:int) : int
      {
         ...
         {
            this.ins_h = (this.ins_h << this.hash_shift ^ this.window[_loc6_ + (MIN_MATCH - 1)] & 0xFF) & this.hash_mask;
      ...
      public function deflate(param1:ZStream, param2:int) : int
      {
         ...
         if(this.status == INIT_STATE)
         {
            ...
            if(this.strstart != 0)
            {
               ...
               this.putShortMSB(int(param1.adler & 0xFFFF));
            }
            param1.adler = param1._adler.adler32(0,null,0,0);
         }
         if(this.pending != 0)
