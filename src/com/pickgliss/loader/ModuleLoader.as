package com.pickgliss.loader
{
   ...
   public class ModuleLoader extends DisplayLoader
   {
      ...
      {
         var _loc2_:Object = loader.init();
         return _loc2_.decry(param1);
      }
      ...
      override protected function __onDataLoadComplete(param1:Event) : void
      {
         ...
         if(DisplayLoader.isDebug)
         {
            ...
            _loc2_ = _loader.data;
            if(_loc2_[0] != 67 || _loc2_[1] != 87 || _loc2_[2] != 83)
            {
               _loc2_ = decry(_loc2_);
            }
            if(domain != null)
            {
               _displayLoader.loadBytes(_loc2_,DisplayLoader.Context);
            }
            else
            {
         ...
         else
         {
            ...
            _loc3_ = _loader.data;
            LoaderSavingManager.cacheFile(_url,_loc3_,false);
            this.analyMd5(_loc3_);
         }
      }
      ...
      public function analyMd5(param1:ByteArray) : void
      {
         ...
         var content:ByteArray = param1;
         var name:String = this.getName();
         if(ComponentSetting.USEMD5 && (ComponentSetting.md5Dic[name] || this.hasHead(content)))
         {
            if(this.compareMD5(content,name))
            {
               try
               {
                  temp = new ByteArray();
