package church.view.churchScene
{
   ...
   public class WeddingSceneMap extends SceneMap
   {
      ...
      public function fireImdily(param1:Point, param2:uint, param3:Boolean = false) : void
      {
         ...
         var _loc4_:int = int(this._model.fireTemplateIDList[param2]);
      ...
      public function playWeddingMovie() : void
      {
         ...
         _loc2_ = ComponentFactory.Instance.creatCustomObject("church.WeddingSceneMap.groomPos");
      ...
      public function rangeGuest() : void
      {
         ...
         var _loc3_:int = 0;
         var _loc4_:uint = 0;
         var _loc5_:ChurchPlayer = null;
         this.getGuestPos();
         var _loc1_:Array = _characters.list;
         _loc1_.sortOn("ID",Array.NUMERIC);
         while(_loc4_ < _characters.length)
         {
            _loc5_ = _loc1_[_loc4_] as ChurchPlayer;
            if(!ChurchManager.instance.isAdmin(_loc5_.playerVO.playerInfo))
            {
               if(_loc5_.playerVO.playerInfo.Sex)
               {
                  _loc3_ = this.guestPos[0].length == 0 ? 1 : 0;
                  _loc5_.x = (this.guestPos[_loc3_][0] as Point).x;
                  _loc5_.y = (this.guestPos[_loc3_][0] as Point).y;
                  (this.guestPos[_loc3_] as Array).shift();
                  _loc5_.sceneCharacterActionType = "naturalStandBack";
                  _loc5_.sceneCharacterDirection = SceneCharacterDirection.RT;
               }
               else
               {
                  _loc3_ = this.guestPos[2].length == 0 ? 3 : 2;
                  _loc5_.x = (this.guestPos[_loc3_][0] as Point).x;
                  _loc5_.y = (this.guestPos[_loc3_][0] as Point).y;
                  (this.guestPos[_loc3_] as Array).shift();
                  _loc5_.sceneCharacterActionType = "naturalStandBack";
                  _loc5_.sceneCharacterDirection = SceneCharacterDirection.LT;
               }
               _loc2_++;
            }
            _loc4_++;
      ...
      private function spliceLine(param1:DisplayObject, param2:uint, param3:Boolean, param4:Boolean) : Array
      {
         ...
         var _loc7_:int = param3 ? 1 : -1;
         var _loc8_:int = param4 ? -1 : 1;
         var _loc9_:Array = [];
      ...
      private function __fireTimer(param1:TimerEvent) : void
      {
         ...
         _loc4_ = !(Math.round(Math.random() * 9) % 3) ? true : false;
