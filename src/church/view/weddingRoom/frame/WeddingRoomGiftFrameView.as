package church.view.weddingRoom.frame
{
   ...
   public class WeddingRoomGiftFrameView extends BaseAlerFrame
   {
      ...
      {
         var _loc1_:uint = Math.floor(PlayerManager.Instance.Self.Money / LEAST_GIFT_MONEY);
         var _loc2_:uint = Math.ceil(Number(this._txtMoney.text) / LEAST_GIFT_MONEY) == 0 ? 1 : uint(Math.ceil(Number(this._txtMoney.text) / LEAST_GIFT_MONEY));
         if(_loc2_ >= _loc1_)
         {
            _loc2_ = _loc1_;
         }
         this._txtMoney.text = String(_loc2_ * LEAST_GIFT_MONEY);
      }
