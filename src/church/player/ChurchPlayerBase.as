package church.player
{
   ...
   public class ChurchPlayerBase extends SceneCharacterPlayerBase
   {
      ...
      private function sceneCharacterLoadBodyNatural() : void
      {
         ...
         this._sceneCharacterLoaderBody = new SceneCharacterLoaderBody(this._playerInfo,this._getSceneCharacterLoaderPath());
         this._sceneCharacterLoaderBody.load(this.sceneCharacterLoaderBodyNaturalCallBack);
      }
      
      private function _getSceneCharacterLoaderPath() : String
      {
         var _loc1_:String = null;
         if(ChurchManager.instance.isAdmin(this._playerInfo))
         {
            if(ChurchManager.instance.currentRoom.roomType == 1)
            {
               _loc1_ = this._playerInfo.Sex ? "cloth1" : "cloth1";
            }
            else
            {
               _loc1_ = this._playerInfo.Sex ? "cloth5" : "cloth5";
            }
         }
         else if(ChurchManager.instance.currentRoom.roomType == 1)
         {
            _loc1_ = this._playerInfo.Sex ? "cloth6" : "cloth7";
         }
         else
         {
            _loc1_ = this._playerInfo.Sex ? "cloth7" : "cloth6";
         }
         return _loc1_;
      }
      
      private function _getSceneCharacterHairLoaderPath() : String
      {
         var _loc1_:String = null;
         ...
         else if(ChurchManager.instance.isAdmin(this._playerInfo))
         {
            if(ChurchManager.instance.currentRoom.roomType == 1)
            {
               _loc1_ = "hair95";
