package calendar.view
{
   ...
   public class SignedAwardHolder extends Sprite implements Disposeable
   {
      ...
      public function setAwardsByCount(param1:int) : void
      {
         ...
         this.clean();
         var _loc2_:Point = ComponentFactory.Instance.creatCustomObject("ddtcalendar.Award.cell.TopLeft");
         var _loc3_:int = 0;
         for each(_loc4_ in this._model.awards)
         {
            if(_loc4_.AwardDays == param1)
            {
               _loc5_ = ComponentFactory.Instance.creatCustomObject("ddtcalendar.SignAwardCell");
               this._awardCells.push(_loc5_);
               if(_loc4_.TemplateID == 1)
               {
                  _loc5_.info = ItemManager.Instance.getTemplateById(ServerConfigManager.instance.dailyRewardIDForMonth[TimeManager.Instance.Now().getMonth()]);
               }
               else
               {
