package calendar.view.goodsExchange
{
   ...
   public class GoodsExchangeView extends Sprite implements Disposeable
   {
      ...
      private function updateHaveGoodsBox(param1:int) : void
      {
         ...
         var _loc2_:int = int(this._goodsExchangeInfoVector.length);
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         while(_loc4_ < _loc2_)
         {
            if(this._goodsExchangeInfoVector[_loc4_].ItemType == param1 * 2)
            {
               this._goodsExchangeInfoVector[_loc4_].Num = this.count;
               _loc5_ = new GoodsExchangeCell(this._goodsExchangeInfoVector[_loc4_]);
               this.getLeastCount(_loc5_);
               this._haveGoodsBox.addChild(_loc5_);
               _loc3_ += 1;
               this._cellVector.push(_loc5_);
            }
            _loc4_++;
         }
         if(_loc3_ < HAVE_GOODS_CELL_COUNT)
         {
            _loc6_ = _loc3_;
            while(_loc6_ < HAVE_GOODS_CELL_COUNT)
            {
      ...
      private function updateExchangeGoodsBox(param1:int) : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            if(this._goodsExchangeInfoVector[_loc4_].ItemType == param1 * 2 + 1)
            {
               this._goodsExchangeInfoVector[_loc4_].Num = this.count;
               this._exchangGoodsBox.addChild(new GoodsExchangeCell(this._goodsExchangeInfoVector[_loc4_],this._activeEventsInfo.ActiveType,false));
               _loc3_ += 1;
            }
            _loc4_++;
         }
      ...
      {
         var _loc4_:SendGoodsExchangeInfo = null;
         var _loc1_:Boolean = false;
         var _loc2_:Vector.<SendGoodsExchangeInfo> = new Vector.<SendGoodsExchangeInfo>();
         var _loc3_:int = 0;
         while(_loc3_ < this._cellVector.length)
         {
            _loc4_ = new SendGoodsExchangeInfo();
            if(!this._cellVector[_loc3_].info || !this._cellVector[_loc3_].itemInfo)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("calendar.view.goodsExchange.warningIII"));
               return;
            }
            _loc4_.id = this._cellVector[_loc3_].info.TemplateID;
            _loc4_.place = this._cellVector[_loc3_].itemInfo.Place;
            _loc4_.bagType = this._cellVector[_loc3_].itemInfo.BagType;
            _loc2_.push(_loc4_);
            if(this._cellVector[_loc3_].needCount - this.count <= 0)
            {
               _loc1_ = true;
            }
            _loc3_++;
         ...
         this._exchangGoodsCount.text = "1";
         SocketManager.Instance.out.sendGoodsExchange(_loc2_,this._activeEventsInfo.ActiveID,this.count,this._awardIndex);
         this._count = 1;
         if(_loc1_)
         {
      ...
      private function getLeastCount(param1:GoodsExchangeCell) : void
      {
         ...
         {
            this._haveGoodsCount = this._haveGoodsCount > _loc2_ ? _loc2_ : this._haveGoodsCount;
         }
      }
