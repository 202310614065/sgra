package consortion
{
   ...
   public class ConsortionModel extends EventDispatcher
   {
      ...
      {
         var _loc1_:Array = [];
         var _loc2_:Array = this.onlineConsortiaMemberList;
         var _loc3_:ConsortiaPlayerInfo = new ConsortiaPlayerInfo();
         _loc3_.type = 0;
         _loc3_.isSelected = true;
         _loc3_.RatifierName = LanguageMgr.GetTranslation("tank.view.common.ConsortiaIcon.self") + " [" + _loc2_.length + "/" + this.memberList.length + "]";
         _loc1_.push(_loc3_);
         return _loc1_;
      }
      ...
      {
         param1 = param1 < 0 ? 1 : (param1 > Math.ceil(this._myApplyList.length / param2) ? int(Math.ceil(this._myApplyList.length / param2)) : param1);
      ...
      public function getLevelString(param1:int, param2:int) : Vector.<String>
      {
         ...
         switch(param1)
         {
            case LEVEL:
               ...
               {
                  _loc4_ = this.getLevelData(param2).Count;
