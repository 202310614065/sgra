package consortionBattle.view
{
   ...
   public class ConsBatChatView extends Sprite implements Disposeable
   {
      ...
      {
         var _loc2_:ConsBatChatViewCell = null;
         this._cellList = [];
         var _loc1_:int = 0;
         while(_loc1_ < 3)
         {
            _loc2_ = new ConsBatChatViewCell();
            _loc2_.index = _loc1_;
            _loc2_.addEventListener(ConsBatChatViewCell.GUARD_COMPLETE,this.guardCompleteHandler,false,0,true);
            _loc2_.addEventListener(ConsBatChatViewCell.DISAPPEAR_COMPLETE,this.disappearCompleteHandler,false,0,true);
            addChild(_loc2_);
