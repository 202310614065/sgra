package consortionBattle.view
{
   ...
   public class ConsBatTwoBtnView extends Sprite implements Disposeable
   {
      ...
      private static const COINS:int = 15;
      ...
      private function autoBuyPowerFull() : void
      {
         if(!ConsortiaBattleManager.instance.isPowerFullUsed)
         {
            if(PlayerManager.Instance.Self.BandMoney >= COINS)
            {
               SocketManager.Instance.out.sendConsBatConsume(1,true);
            }
            else if(PlayerManager.Instance.Self.Money >= COINS)
            {
      ...
      private function powerFullHandler(param1:MouseEvent) : void
      {
         ...
         {
            if(Boolean(_loc2_.isBand) && PlayerManager.Instance.Self.BandMoney < COINS)
            {
            ...
            {
               if(!(!_loc2_.isBand && PlayerManager.Instance.Self.Money < COINS))
               {
      ...
      private function __powerFullConfirm(param1:FrameEvent) : void
      {
         ...
         SoundManager.instance.play("008");
         var _loc2_:BaseAlerFrame = param1.currentTarget as BaseAlerFrame;
         _loc2_.removeEventListener(FrameEvent.RESPONSE,this.__powerFullConfirm);
         if(param1.responseCode == FrameEvent.SUBMIT_CLICK || param1.responseCode == FrameEvent.ENTER_CLICK)
         {
            if(_loc2_.isBand && PlayerManager.Instance.Self.BandMoney < COINS)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.consortiaBattle.buy.noEnoughBindMoneyTxt"));
               return;
            }
            ...
            {
               _loc3_ = ConsortiaBattleManager.instance.getBuyRecordStatus(0);
      ...
      private function __doubleScoreConfirm(param1:FrameEvent) : void
      {
         ...
         if(param1.responseCode == FrameEvent.SUBMIT_CLICK || param1.responseCode == FrameEvent.ENTER_CLICK)
         {
            if(_loc2_.isBand && PlayerManager.Instance.Self.BandMoney < COINS * 2)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.consortiaBattle.buy.noEnoughBindMoneyTxt"));
               return;
            }
            if(!_loc2_.isBand && PlayerManager.Instance.Self.Money < 150)
            {
               LeavePageManager.showFillFrame();
               return;
            }
