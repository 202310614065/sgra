package consortionBattle.view
{
   ...
   public class ConsBatResurrectView extends Sprite implements Disposeable
   {
      ...
      protected function promptlyStayRevive() : void
      {
         ...
         {
            if(Boolean(_loc1_.isBand) && PlayerManager.Instance.Self.BandMoney < 350)
            {
            ...
            {
               if(!(!_loc1_.isBand && PlayerManager.Instance.Self.Money < 350))
               {
      ...
      protected function __StayResurrectConfirm(param1:FrameEvent) : void
      {
         ...
         _loc2_.removeEventListener(FrameEvent.RESPONSE,this.__StayResurrectConfirm);
         if(param1.responseCode == FrameEvent.SUBMIT_CLICK || param1.responseCode == FrameEvent.ENTER_CLICK)
         {
            if(_loc2_.isBand && PlayerManager.Instance.Self.BandMoney < 50)
            {
               _loc3_ = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("ddt.consortiaBattle.changeMoneyCostTxt"),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),true,true,true,LayerManager.BLCAK_BLOCKGOUND);
               _loc3_.moveEnable = false;
               _loc3_.addEventListener(FrameEvent.RESPONSE,this.__StayResurrectTwoConfirm,false,0,true);
               return;
            }
            ...
            {
               _loc4_ = ConsortiaBattleManager.instance.getBuyRecordStatus(3);
               _loc4_.isNoPrompt = true;
               _loc4_.isBand = _loc2_.isBand;
            }
            SocketManager.Instance.out.sendConsBatConsume(4,_loc2_.isBand);
         }
      }
      ...
      protected function promptlyRevive() : void
      {
         ...
         if(Boolean(_loc1_.isNoPrompt))
         {
            if(Boolean(_loc1_.isBand) && PlayerManager.Instance.Self.BandMoney < 5)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.consortiaBattle.buy.noEnoughBindMoneyTxt"));
               _loc1_.isNoPrompt = false;
            }
            ...
            {
               if(!(!_loc1_.isBand && PlayerManager.Instance.Self.Money < 5))
               {
         ...
         var _loc2_:BaseAlerFrame = AlertManager.Instance.simpleAlert(LanguageMgr.GetTranslation("AlertDialog.Info"),LanguageMgr.GetTranslation("ddt.consortiaBattle.buyResurrect.promptTxt",5),LanguageMgr.GetTranslation("ok"),LanguageMgr.GetTranslation("cancel"),true,true,true,LayerManager.BLCAK_BLOCKGOUND,null,"ConsBatBuyConfirmView",30,true);
         _loc2_.moveEnable = false;
      ...
      protected function __resurrectConfirm(param1:FrameEvent) : void
      {
         ...
         if(param1.responseCode == FrameEvent.SUBMIT_CLICK || param1.responseCode == FrameEvent.ENTER_CLICK)
         {
            if(_loc2_.isBand && PlayerManager.Instance.Self.BandMoney < 5)
            {
               MessageTipManager.getInstance().show(LanguageMgr.GetTranslation("ddt.consortiaBattle.buy.noEnoughBindMoneyTxt"));
               return;
            }
            if(!_loc2_.isBand && PlayerManager.Instance.Self.Money < 5)
            {
               LeavePageManager.showFillFrame();
               return;
            }
            ...
            SocketManager.Instance.out.sendConsBatConsume(3,_loc2_.isBand);
         }
      }
