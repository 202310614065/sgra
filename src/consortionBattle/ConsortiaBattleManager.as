package consortionBattle
{
   ...
   public class ConsortiaBattleManager extends EventDispatcher
   {
      ...
      public function judgeCreatePlayer(param1:Number, param2:Number) : void
      {
         ...
         var _loc8_:ConsortiaBattlePlayerInfo = null;
         var _loc3_:Array = [];
         for each(_loc4_ in this._buffPlayerList)
         {
         ...
         {
            _loc8_ = this._buffPlayerList[_loc5_];
            this._buffPlayerList.remove(_loc5_);
            this._buffCreatePlayerList.add(_loc8_.id,_loc8_);
         }
         if(_loc3_.length > 0 && !this._timer.running)
         {
            this._timer.start();
         }
      }
      ...
      private function updatePlayerStatus(param1:PackageIn) : void
      {
         ...
         if(!this.isInMainView && _loc2_ != PlayerManager.Instance.Self.ID)
         {
            return;
         }
         if(Boolean(this._playerDataList[_loc2_]))
         {
            _loc3_ = this._playerDataList[_loc2_];
         }
         else if(Boolean(this._buffPlayerList[_loc2_]))
         {
            _loc3_ = this._buffPlayerList[_loc2_];
         }
         else
         {
            _loc3_ = this._buffCreatePlayerList[_loc2_];
         }
         if(Boolean(_loc3_))
         {
            ...
            if(_loc3_ == this._playerDataList[_loc2_])
            {
               this._playerDataList.add(_loc3_.id,_loc3_);
            }
         }
      }
      ...
      private function movePlayer(param1:PackageIn) : void
      {
         ...
         var _loc10_:ConsBatEvent = null;
         var _loc11_:ConsortiaBattlePlayerInfo = null;
         if(!this.isInMainView)
         {
            return;
         }
         ...
         if(Boolean(this._playerDataList[_loc2_]))
         {
            _loc10_ = new ConsBatEvent(MOVE_PLAYER);
            _loc10_.data = {
               "id":_loc2_,
               "path":_loc7_
            };
         ...
         {
            if(Boolean(this._buffPlayerList[_loc2_]))
            {
               _loc11_ = this._buffPlayerList[_loc2_];
            }
            else
            {
               _loc11_ = this._buffCreatePlayerList[_loc2_];
            }
            if(Boolean(_loc11_))
            {
               _loc11_.pos = _loc7_[_loc7_.length - 1];
            }
         }
      }
      ...
      private function addPlayerInfo(param1:PackageIn) : void
      {
         ...
         while(_loc3_ < _loc2_)
         {
            _loc4_ = new ConsortiaBattlePlayerInfo();
            _loc4_.id = param1.readInt();
            _loc4_.tombstoneEndTime = param1.readDate();
            _loc4_.status = param1.readByte();
            _loc4_.pos = new Point(param1.readInt(),param1.readInt());
            _loc4_.sex = param1.readBoolean();
            _loc5_ = param1.readInt();
            if(_loc5_ == PlayerManager.Instance.Self.ConsortiaID)
            {
               _loc4_.selfOrEnemy = 1;
            }
            else
            {
               _loc4_.selfOrEnemy = 2;
            }
            _loc4_.clothIndex = 2;
            _loc4_.consortiaName = param1.readUTF();
            _loc4_.winningStreak = param1.readInt();
            _loc4_.failBuffCount = param1.readInt();
            if(_loc4_.id != PlayerManager.Instance.Self.ID)
            {
               this._buffPlayerList.add(_loc4_.id,_loc4_);
            }
            _loc3_++;
         }
      }
      ...
      {
         var _loc1_:BaseLoader = LoadResourceManager.Instance.createLoader(this.resourcePrUrl + "map/factionwarmap.swf",BaseLoader.MODULE_LOADER);
      ...
      {
         return LoadResourceManager.Instance.createLoader(param1,BaseLoader.BITMAP_LOADER);
      }
   }
}
