package bagAndInfo.cell
{
   ...
   public class PersonalInfoCell extends BagCell
   {
      ...
      override public function dragDrop(param1:DragEffect) : void
      {
         ...
         if(Boolean(_loc2_))
         {
            ...
            if(PlayerManager.Instance.Self.canEquip(_loc2_))
            {
               ...
               {
                  _loc4_ = PlayerManager.Instance.getDressEquipPlace(_loc2_);
               }
               if(!PetBagController.instance().petModel.currentPetInfo)
               {
                  SocketManager.Instance.out.sendMoveGoods(BagInfo.EQUIPBAG,_loc2_.Place,BagInfo.EQUIPBAG,_loc4_,_loc2_.Count);
                  param1.action = DragEffect.NONE;
      ...
      private function __onBindResponse(param1:FrameEvent) : void
      {
         SoundManager.instance.play("008");
         var _loc2_:BaseAlerFrame = param1.target as BaseAlerFrame;
         _loc2_.removeEventListener(FrameEvent.RESPONSE,this.__onBindResponse);
         _loc2_.dispose();
         if(param1.responseCode == FrameEvent.ENTER_CLICK || param1.responseCode == FrameEvent.SUBMIT_CLICK)
         {
            this.sendBindDefy();
