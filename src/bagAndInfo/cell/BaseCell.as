      ...
      public function set info(param1:ItemTemplateInfo) : void
      {
         ...
         if(Boolean(this._info))
         {
            ...
            else if(this._info.CategoryID != EquipType.CARDEQUIP)
            {
               ...
               _loc2_ = this.info as InventoryItemInfo;
               if(this.info.Property1 == "31")
               {
                  if(Boolean(_loc2_) && _loc2_.Hole2 > 0)
                  {
                     GoodTipInfo(this._tipData).exp = _loc2_.Hole2;
                     GoodTipInfo(this._tipData).upExp = ServerConfigManager.instance.getBeadUpgradeExp()[_loc2_.Hole1 + 1];
                     GoodTipInfo(this._tipData).beadName = _loc2_.Name + "-" + beadSystemManager.Instance.getBeadName(_loc2_);
                  }
                  else
                  {
                     ...
                     GoodTipInfo(this._tipData).beadName = param1.Name + "-" + BeadTemplateManager.Instance.GetBeadInfobyID(param1.TemplateID).Name + "Lv" + BeadTemplateManager.Instance.GetBeadInfobyID(this.info.TemplateID).BaseLevel;
                  }
               }
               else if(this.info.Property1 == "81")
               {
                  if(Boolean(_loc2_) && _loc2_.StrengthenExp > 0)
                  {
                     GoodTipInfo(this._tipData).exp = _loc2_.StrengthenExp - MagicStoneManager.instance.getNeedExp(this.info.TemplateID,_loc2_.StrengthenLevel);
                  }
                  else
                  {
