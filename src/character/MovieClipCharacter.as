package character
{
   ...
   public class MovieClipCharacter extends Sprite implements ICharacter
   {
      ...
      public function set description(des:XML) : void
      {
         ...
         if(Boolean(des.hasOwnProperty("@registerX")))
         {
            this._registerPoint.x = des.@registerX;
         }
         if(Boolean(des.hasOwnProperty("@registerY")))
         {
            this._registerPoint.y = des.@registerY;
         }
         if(Boolean(des.hasOwnProperty("@rect")))
         {
            r = String(des.@rect);
         ...
         var _loc3_:int = 0;
         for(var _loc4_:* = actions; §§hasnext(_loc4_,_loc3_); a = new MovieClipAction(resource,action.@name,action.@next,action.@priority),a.endStop = String(action.@endStop) == "true",a.sound = action.@sound,this._actionSet.addAction(a))
         {
            action = §§nextvalue(_loc3_,_loc4_);
