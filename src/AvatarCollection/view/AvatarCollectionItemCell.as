package AvatarCollection.view
{
   ...
   public class AvatarCollectionItemCell extends Sprite implements Disposeable
   {
      ...
      private function buyClickHandler(param1:MouseEvent) : void
      {
         ...
         var _loc2_:ShopItemInfo = ShopManager.Instance.getGoodsByTemplateID(this._data.itemId);
         this.buy(_loc2_.GoodsID,_loc2_.isDiscount,_loc2_.getItemPrice(1).PriceType);
      ...
      {
         this.view = new BuySingleGoodsView2(param3);
         LayerManager.Instance.addToLayer(this.view,LayerManager.GAME_DYNAMIC_LAYER,true,LayerManager.BLCAK_BLOCKGOUND);
         BuySingleGoodsView2(this.view).isDisCount = param2 == 1 ? false : true;
         BuySingleGoodsView2(this.view).isSale = param4;
         BuySingleGoodsView2(this.view).goodsID = param1;
      }
      
      private function onBuyConfirmResponse(param1:MouseEvent) : void
      {
         var _loc2_:Boolean = false;
         if(_loc2_ && PlayerManager.Instance.Self.BandMoney < this._data.buyPrice)
         {
         ...
         var _loc3_:Array = [this._data.goodsId];
         var _loc4_:Array = [1];
         var _loc5_:Array = [""];
         var _loc6_:Array = [""];
         var _loc7_:Array = [""];
         var _loc8_:Array = [this._data.isDiscount];
         var _loc9_:Array = [false];
         this.view.dispose();
      }
